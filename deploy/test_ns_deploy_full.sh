...
assert_contains 'pnpm --dir "$BLOSSOM_NEW" install --prod --frozen-lockfile' "$DEPLOY_SCRIPT"
assert_contains 'sudo mv "$BLOSSOM_NEW/node_modules" "$BLOSSOM_REMOTE_DIR/node_modules"' "$DEPLOY_SCRIPT"
assert_contains 'sudo flock -n /var/lock/nostr-ltd-deploy.lock' "$DEPLOY_SCRIPT"
